...
--EXTRACT DATA FOR TRAINING/MODEL
SELECT E.CNTR_NUM        
,E.CNTR_TEU                
,DECODE(E.CNTR_SIZE,'20',1,0) IS_20
...
,CASE WHEN E.CNTR_TYPE='GP' THEN 0 
      WHEN E.CNTR_TYPE='HQ' THEN 0 
  ELSE 1 END IS_XX
,CASE WHEN E.TARIFF_CODE='096' THEN 3 WHEN E.TARIFF_CODE='052' THEN 2 ELSE 1 END TARIFF_CODE                         
...
,E.LADEN_PICKUP_FCIL_CDE
,E.MTY_RTN_FCIL_CDE
,ROUND( MTY_RTN_TRANSITTIME_ACTUAL ) MTY_RTN_TRANSITTIME_ACTUAL              
FROM EQPRO_NAG_IB_BKG_PRED E,SFT_CMS_BASIC_INFO SFT
WHERE E.SNAPSHOT_ID=1739 
/*NON OVERWEIGHT CNTRS*/
AND 0 =(CASE WHEN (E.CNTR_SIZE='20' AND E.VGM_WT > 17000) 
         THEN 1 
       WHEN (E.CNTR_SIZE!='20' AND E.VGM_WT > 20000) 
         THEN 1 ELSE 0 END)  
AND SFT.MVMT_SPEC_REF_NUM=E.MVMT_SPEC_REF_NUM
/*FULL PICKUP AND EMPTY RETURN LOCATION IS LGB10 */
AND  E.LADEN_PICKUP_FCIL_CDE='LGB10' 
AND E.MTY_RTN_FCIL_CDE='LGB10'   
AND E.BL_CCP_CDE=7297147000
AND MTY_RTN_TRANSITTIME_ACTUAL BETWEEN  3*24
    AND  
     ( SELECT MAX_AVG_TT_DAYS*24
        FROM EQPRO_NAG_IB_BKG_CUST_PRED 
        WHERE BL_CCP_CDE=E.BL_CCP_CDE    
   );
